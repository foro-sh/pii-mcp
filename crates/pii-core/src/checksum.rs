...
// SWIFT IBAN registry: country code -> fixed IBAN length. Sorted for binary_search.
#[rustfmt::skip]
...
/// IBAN mod-97 plus registry length, after compacting whitespace/hyphens/slashes/soft-hyphens
/// and uppercasing.
pub fn iban_valid(value: &str) -> bool {
    ...
    // Registry length for the country: a hex digest slice passes mod-97 alone
    // about one time in 97.
    let Ok(i) = IBAN_LENGTHS.binary_search_by(|(cc, _)| (*cc).cmp(&compact[..2])) else {
        return false;
    };
    if IBAN_LENGTHS[i].1 != len {
        return false;
    }
